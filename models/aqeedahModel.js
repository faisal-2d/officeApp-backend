...
async function getLeaderboard(batch_no, level) {
    const options = {
        sort: { [`aqeedah${level}Total`]: -1 }
      }; 
    const db = await connectDB("aqeedah_16");
    const users = db.collection(`aqeedah_${batch_no}_list`);    
    const result = await users.find({[`aqeedah${level}data`]:{$ne:null}}, options).toArray();
    return result;     
}
...
async function certificateUpload(batch_no, level, sn, certificate) {
    const filter = {sn : sn};
    const updateDocument = {
        $set : {[`aqeedahCourseCertificates.level${level}`] : certificate},
      } 
    const db = await connectDB("aqeedah_16");
    const users = db.collection(`aqeedah_${batch_no}_list`);  
    const result = await users.updateOne(filter, updateDocument);      
