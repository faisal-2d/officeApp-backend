...
async function registerStudent(student, batch_no) {
    ...
    const db = await connectDB("fiqh");
    const users = db.collection(`fiqh_${batch_no}_list`);
...
async function updateResult(batch_no, level, exm, sn, score) {
    ...
    const updateDocument = {
        $set : { [`fiqh${level}data.${[exm-1]}`] : score } ,
      }
    const db = await connectDB("fiqh");
    const users = db.collection(`fiqh_${batch_no}_list`);
...
async function updateLeaderboard(batch_no, level, sn) {
    ...
    const scores = `fiqh${level}data`;
    const totalScores = `fiqh${level}Total`;
...
async function getLeaderboard(batch_no, level) {
    const options = {
        sort: { [`fiqh${level}Total`]: -1 }
      }; 
    const db = await connectDB("fiqh");
    const users = db.collection(`fiqh_${batch_no}_list`);    
    const result = await users.find({[`fiqh${level}data`]:{$ne:null}}, options).toArray();
    return result;     
}
...
async function certificateUpload(batch_no, level, sn, certificate) {
    const filter = {sn : sn};
    const updateDocument = {
        $set : {[`fiqhCourseCertificates.level${level}`] : certificate},
      } 
    const db = await connectDB("fiqh");
    const users = db.collection(`fiqh_${batch_no}_list`);  
    const result = await users.updateOne(filter, updateDocument);      
